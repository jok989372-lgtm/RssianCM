...
public sealed partial class MineDetectorOverlaySystem : EntitySystem
{
    ...
    {
        if (!_overlay.HasOverlay<MineDetectorOverlay>())
            _overlay.AddOverlay(new MineDetectorOverlay());
    }

    public override void Shutdown()
    {
        _overlay.RemoveOverlay<MineDetectorOverlay>();
    }
    ...
    public void DrawBlips<T>(DrawingHandleWorld handle, ref TimeSpan last, List<(Vector2 Pos, bool QueenEye)> blips, Texture texture, Texture queenEyeTexture) where T : IComponent, IDetectorComponent
    {
        ...
        foreach (var held in ents)
        {
            ...
            {
                handle.DrawTexture(diff.QueenEye ? queenEyeTexture : texture, playerCoords.Position + diff.Pos, Color.Red);
            }
        }
    }
